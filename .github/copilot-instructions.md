# Cliply AI Coding Instructions

## Project Overview
Cliply is a Next.js 15 marketplace connecting brands and creators with trackable payouts. Uses App Router, MongoDB, custom JWT auth, and role-based access control.

## Agent Rules
You are an AI coding assistant. You operate in Visual Studio Code. 
...
- **Custom JWT implementation**: Uses `src/lib/auth.ts` for client-side auth hook with localStorage tokens
- **Cookie-based middleware**: `src/middleware.ts` handles role-based redirects (`/creator` vs `/brand` routes)
- **API token verification**: Auth headers use `Bearer ${token}` format with base64-encoded JWT payload
- **Password hashing**: Uses bcryptjs for password encryption in auth endpoints

### Database Layer
- **MongoDB with native driver**: `src/lib/mongodb.ts` provides connection pooling and database access
- **Schema definitions**: TypeScript interfaces in `src/lib/mongodb/schemas.ts` (not Mongoose models)
- **Helper functions**: `src/lib/db.ts` contains CRUD operations like `createCampaign()`, `getCampaigns()`
- **ObjectId handling**: Always convert string IDs to `new ObjectId()` when querying MongoDB

### Route Organization
- **Role-based route groups**: `(dashboard)`, `(legal)`, `(marketing)` in app directory
- **API structure**: `/api/auth/`, `/api/campaigns/`, `/api/brands/` with dedicated route handlers
- **Protected routes**: Middleware redirects based on `user_role` cookie and `auth_token`

### Validation & Types
- **Zod schemas**: `src/lib/validators.ts` defines API request validation (e.g., `campaignCreateSchema`)
- **TypeScript interfaces**: Campaign, Profile, Brand types in `src/lib/mongodb/schemas.ts`
- **Form validation**: Uses `@hookform/resolvers` with Zod for client-side forms

### Component Patterns
- **Feature-based organization**: `src/components/features/dashboard/`, `src/components/features/discover/`
- **Layout components**: Role-specific sidebars in `src/components/layouts/`
- **UI library**: Radix UI + Tailwind with `src/components/ui/` shadcn components
- **Utility function**: `cn()` in `src/lib/utils.ts` for conditional CSS classes
...
npm run setup:phase1           # Project initialization script
```

### Environment Variables
- MongoDB: `MONGODB_URI`, `MONGODB_DB`
- Auth: Custom JWT (no NextAuth.js)
- Stripe: `STRIPE_SECRET_KEY`, webhook integration
- Sentry: Optional error tracking

### Database Operations
- Use `getDatabase()` from `src/lib/mongodb.ts` for direct MongoDB operations
- Campaign creation requires `brand_id` conversion to ObjectId
- Collections are defined in `Collections` constant (PROFILES, CAMPAIGNS, etc.)
- **MongoDB Atlas SSL**: Configuration auto-detects Atlas vs local MongoDB for SSL settings
- **Connection troubleshooting**: Check Network Access whitelist in Atlas dashboard
...
### Role-Based Routing
- Creator routes: `/creator/*` → CreatorDashboardSidebar navigation
- Brand routes: `/brand/*` → Brand dashboard layouts
- Admin routes: `/admin/*` → Admin-specific functionality

### Campaign State Management
- Status enum: `'draft' | 'active' | 'paused' | 'completed' | 'deleted'`
- Rate types: `'per_thousand_views' | 'fixed_fee' | 'hybrid' | 'commission'`
- Platform support: YouTube, TikTok, Instagram

### File Naming
- Route handlers: `route.ts` in app directory
- Page components: `page.tsx` 
- Layout components: `layout.tsx`
- Feature components: Descriptive names in feature folders

## Integration Points
- **Stripe**: Payment processing and webhook handling
- **YouTube API**: Creator verification and analytics
- **Sentry**: Error tracking and monitoring
- **Vercel**: Analytics and cron jobs for payout computation

When working with this codebase, always check the existing patterns in `src/lib/db.ts` for database operations and `src/lib/validators.ts` for request validation before creating new functionality.
